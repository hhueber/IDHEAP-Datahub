...
export const authService = {
    async login(
        ...
        if (typeof r.refresh_in !== "number")
            throw new Error("Missing refresh_in in /auth/login response");
    ...
    async refresh(): Promise<RefreshResp> {
        const r = await apiFetch<RefreshResp>("/auth/refresh", {
            method: "POST",
            auth: true,
        });
        if (typeof r.refresh_in !== "number")
            throw new Error("Missing refresh_in in /auth/refresh response");
        saveNextRefresh(r.refresh_in);
        return r;
    },
    ...
    getCachedUser(): User | null {
        const v = localStorage.getItem("user_data");
