...
function DistributionChart({
    ...
    const compactMode = processedData.length > 8 || hasLongLabels;
        ...
        >
            ...
            >
                {processedData.map((d, i) => {
                    ...
                    return (
                        ...
                        >
                            <div
                                className="rounded-lg transition-all duration-300 mt-auto"
                                style={{
                                    width: `${barWidth * 4}px`,
                                    height: `${height}%`,
                                    minHeight: "6px",
            ...
            {gradientLike &&
                (() => {
                    ...
                    return (
                        <div
                            className="relative mt-3 h-4 text-[10px]"
            ...
            {tooltip && (
                ...
                >
                    ...
                    <div>
                        {t("stats.value")} {tooltip.value}
                    </div>
                    <div>
                        {t("stats.count")} {tooltip.count}
                    </div>
                    <div>
                        {t("stats.share")} {tooltip.percentage}
                        {"\u0025"}
                    </div>{" "}
                    {/* Unicode du pourcentage */}
                </div>
            )}

            {/* Legend */}
            <div
                className="flex justify-center gap-4 text-xs mt-4"
                style={{ color: hoverText07 }}
            >
        ...
        >
            <div className="flex justify-between items-center mb-5">
                ...
                <button
                    ...
                    onMouseLeave={(e) =>
                        (e.currentTarget.style.color = hoverText07)
                    }
                >
                    {"\u2715"} {/* Unicode croix */}
                </button>
                        ...
                        {data.context && (
                            ...
                            >
                                {data.context.commune && (
                                    <div>
                                        {t("stats.context.commune")}:{" "}
                                        <strong>{data.context.commune}</strong>
                                    </div>
                                )}

                                {data.context.district && (
                                    <div>
                                        {t("stats.context.district")}:{" "}
                                        <strong>{data.context.district}</strong>
                                    </div>
                                )}

                                {data.context.canton && (
                                    <div>
                                        {t("stats.context.canton")}:{" "}
                                        <strong>{data.context.canton}</strong>
                                    </div>
                                )}

                                {data.context.nb_communes && (
                                    <div>
                                        {t("stats.context.communes", {
                                            count: data.context.nb_communes,
                                        })}
                                    </div>
                                )}
                            </div>
