...
export default function InsightsPanel({ insights, onChildShow }: Props) {
    ...
    const { background, borderColor, textColor, hoverPrimary04, hoverText07 } =
        useTheme();
    ...
    if (hasMap)
        tabs.push({
            key: "map",
            label: t("dashboardSidebar.pageShow.insightsPanel.map"),
        });
    if (hasStats)
        tabs.push({
            key: "stats",
            label: t("dashboardSidebar.pageShow.insightsPanel.stats"),
        });
                ...
                {tabs.map((item) => (
                    <button
                        ...
                        style={{
                            borderColor,
                            backgroundColor:
                                tab === item.key ? hoverPrimary04 : background,
                            color: textColor,
                        }}
                    >
                        {item.label}
                ...
                >
                    <div className="px-4 py-3 border-b" style={{ borderColor }}>
                        ...
                        <div className="text-xs" style={{ color: hoverText07 }}>
                            {t(
                                "dashboardSidebar.pageShow.insightsPanel.mapContext",
                            )}
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-2">
                        {insights?.stats?.items?.map((item, index) => (
                            ...
                            >
                                ...
                                >
                                    {t(
                                        `dashboardSidebar.pageShow.insightsPanel.stat.${item.label_key}`,
                                    )}
                                </span>
                                <span
                                ...
                                >
                                    {typeof item.value === "boolean"
                                        ? item.value
                                            ? t(
                                                  "dashboardSidebar.pageShow.visibility.private",
                                              )
                                            : t(
                                                  "dashboardSidebar.pageShow.visibility.public",
                                              )
                                        : item.value === null ||
