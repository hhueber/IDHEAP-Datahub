...
import {
    PlaceOfInterestApi,
    PlaceOfInterestMapDTO,
    PlaceOfInterestGeoType,
...
const isValidGeoType = (value: unknown): value is PlaceOfInterestGeoType =>
    value === "commune" || value === "district" || value === "canton";

const isValidLocalPlaceOfInterest = (
    value: unknown,
): value is PlaceOfInterestMarker => {
    ...
    return (
        ...
        (item.geoCode === undefined || typeof item.geoCode === "string") &&
        (item.names === undefined || isValidLocalizedPlaceNames(item.names)) &&
        Array.isArray(pos) &&
        pos.length === 2 &&
        typeof pos[0] === "number" &&
        typeof pos[1] === "number" &&
...
const resolveLocalPlaceOfInterestName = (
    ...
    const localizedName =
        placeOfInterest.names?.[lang as keyof LocalizedPlaceNames];
...
export function usePlaceOfInterestMarkers(
    lang: string,
): UsePlaceOfInterestMarkersResult {
    const { t } = useTranslation();
    const [backendPlaceOfInterest, setBackendPlaceOfInterest] = useState<
        PlaceOfInterestMarker[]
    >([]);
    const [extraPlaceOfInterest, setExtraPlaceOfInterest] = useState<
        PlaceOfInterestMarker[]
    >(() => loadLocalPlaceOfInterest());
    ...
    useEffect(() => {
        ...
        PlaceOfInterestApi.list(normalizedLang, ctrl.signal)
            ...
            .catch((e: any) => {
                ...
                setError(
                    e?.message || t("map.errors.failedToLoadPlaceOfInterest"),
                );
            })
            .finally(() => {
     ...
     */
    const addExtraPlaceOfInterest = (
        c: Omit<PlaceOfInterestMarker, "source">,
    ) => {
    ...
    const placeOfInterest = useMemo(() => {
        ...
        const visibleBackend = backendPlaceOfInterest.filter(
            (placeOfInterest) => !hiddenCodes.has(placeOfInterest.code),
        );
