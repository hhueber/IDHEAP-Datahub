...
export default function Dashboard() {
    ...
    const role = user?.role
        ? String(user.role)
        : t("dashboard.roleUnknown", "Utilisateur");

    // placeholders KPI
    const kpis: Kpi[] = [
        {
            label: t("dashboard.kpi.surveys", "Questionnaires"),
            value: "—",
            sub: t("dashboard.kpi.surveysSub", "Publiés / Brouillons"),
        },
        {
            label: t("dashboard.kpi.questions", "Questions"),
            value: "—",
            sub: t("dashboard.kpi.questionsSub", "Nombre total de questions"),
        },
        {
            label: t("dashboard.kpi.answers", "Réponses"),
            value: "—",
            sub: t("dashboard.kpi.answersSub", "Réponses validées"),
        },
        {
            label: t("dashboard.kpi.places", "Lieux suisses"),
            value: "—",
            sub: t("dashboard.kpi.placesSub", "Cantons / Districts / Communes"),
        },
    ];
        ...
        <section className="space-y-6" style={{ color: textColor }}>
            ...
            >
                <div className="flex items-start justify-between gap-4 flex-wrap">
                    <div className="min-w-0">
                        ...
                        >
                            {t("dashboard.mockNotice", "Dashboard mock")}
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Pill
                            ...
                            color={hoverText07}
                        >
                            {t("dashboard.env", "Environnement")}:{" "}
                            {t("dashboard.envValue", "Dev")}
                        </Pill>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
                ...
                <Card bg={cardBg} border={borderColor}>
                    ...
                    >
                        {t(
                            "dashboard.chart2.note",
                            "Plus tard : calculer le pourcentage via les tables de traduction.",
                        )}
                    </div>
                </Card>
                        ...
                        <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
                            ...
                            >
                                <div className="text-sm font-semibold">
                                    {t(
                                        "dashboard.quickActions.newSurvey",
                                        "Créer un questionnaire",
                                    )}
                                </div>
                                <div className="mt-1 text-xs opacity-70">
                                    {t(
                                        "dashboard.quickActions.newSurveySub",
                                        "Nouveau formulaire",
                                    )}
                                </div>
                            </Link>
                            ...
                            >
                                <div className="text-sm font-semibold">
                                    {t(
                                        "dashboard.quickActions.manageQuestions",
                                        "Gérer les questions",
                                    )}
                                </div>
                                <div className="mt-1 text-xs opacity-70">
                                    {t(
                                        "dashboard.quickActions.manageQuestionsSub",
                                        "Banque de questions",
                                    )}
                                </div>
                            </Link>
                            ...
                            >
                                <div className="text-sm font-semibold">
                                    {t(
                                        "dashboard.quickActions.browsePlaces",
                                        "Communes suisses",
                                    )}
                                </div>
                                <div className="mt-1 text-xs opacity-70">
                                    {t(
                                        "dashboard.quickActions.browsePlacesSub",
                ...
                <Card bg={cardBg} border={borderColor}>
                        ...
                        {[
                            {
                                ...
                                status: t("dashboard.health.ok", "OK"),
                                kind: "ok" as const,
                            },
                            {
                                label: t(
                                    "dashboard.health.config",
                                    "Configuration",
                                ),
                                status: t("dashboard.health.ok", "OK"),
                                kind: "ok" as const,
                            ...
                            {
                                label: t(
                                    "dashboard.health.translations",
                                    "Traductions",
                                ),
                                status: t("dashboard.health.todo", "À faire"),
                                kind: "todo" as const,
                            },
                        ].map((row) => {
                    ...
                    >
                        <div
                            ...
                            style={{ backgroundColor: hoverPrimary30 }}
                        />
                    </div>
                    ...
                    >
                        {t(
                            ...
                            "Plus tard : brancher un endpoint /systéme.",
                        )}
                    </div>
                </Card>
                ...
                <Card bg={cardBg} border={borderColor}>
                    ...
                    >
                        {[
                            t(
                                "dashboard.tasks.li1",
                                "Créer /dashboard/stats (compteurs par table).",
                            ),
                            t(
                                "dashboard.tasks.li2",
                                "Calculer la couverture des traductions (clés manquantes).",
                            ),
                            t(
                                "dashboard.tasks.li3",
                                "Ajouter un endpoint /health (DB + config).",
                            ),
                        ].map((task) => (
                    ...
                    >
                        {t(
                            "dashboard.tasks.note",
                            "Liste statique pour l’instant a rendre dynamique plus tard.",
                        )}
                    </div>
                </Card>
            </div>
