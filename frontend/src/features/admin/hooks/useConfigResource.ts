...
export function useConfigResource<T>(api: {
    ...
    const reload = async () => {
        ...
        try {
            ...
            else setErr(r.detail || t("config.errors.loadFailed"));
        } catch {
            setErr(t("config.errors.network"));
        } finally {
            setLoading(false);
        }
    };
    ...
    const save = async (body: T) => {
        ...
        if (r.success) {
            setMsg(r.detail || t("config.errors.saved"));
            await reload();
        } else setErr(r.detail || t("config.errors.saveFailed"));
    };
    ...
    const remove = async (code: string) => {
        ...
        if (r.success) {
            setMsg(r.detail || t("config.errors.deleted"));
            await reload();
        } else setErr(r.detail || t("config.errors.deleteFailed"));
    };
