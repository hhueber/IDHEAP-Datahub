...
export default function AddMemberPage() {
    ...
    const validate = () => {
        ...
        if (form.password !== form.confirm) {
            setErrKey("admin.addMember.errors.confirmMismatch");
            return false;
        }
        // mot de passe trop court coté client
        if (form.password.length < 10) {
            setErrKey("admin.addMember.errors.tooShort");
            return false;
        }
        return true;
    };
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (Array.isArray(d))
                setErrKey("admin.addMember.errors.serverValidation");
            // email déjà utilisé
            else if (ae?.status === 409)
                setErrKey("admin.addMember.errors.emailExists");
            // action interdite
            else if (ae?.status === 403)
                setErrKey("admin.addMember.errors.forbidden");
            // erreur générique
        ...
        >
            ...
            <form className="space-y-4" onSubmit={onSubmit}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium mb-1">
                            {t("admin.addMember.firstNameLabel")}
                        </label>
                        <input
                            name="first_name"
                            ...
                            placeholder={t(
                                "admin.addMember.firstNamePlaceholder",
                            )}
                    ...
                    <div>
                        <label className="block text-sm font-medium mb-1">
                            {t("admin.addMember.lastNameLabel")}
                        </label>
                        <input
                            name="last_name"
                            ...
                            className="w-full rounded-lg border px-3 py-2"
                            placeholder={t(
                                "admin.addMember.lastNamePlaceholder",
                            )}
                ...
                <div>
                    ...
                    >
                        <option value="MEMBER">
                            {t("admin.addMember.roles.member")}
                        </option>
                        <option value="ADMIN">
                            {t("admin.addMember.roles.admin")}
                        </option>
                    </select>
                </div>
                {/* Mot de passe */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <PasswordField
                            ...
                            placeholder={t(
                                "admin.addMember.passwordPlaceholder",
                            )}
                            value={form.password}
                            onChange={onChange}
                        ...
                        >
                            {t("admin.addMember.passwordHelp")}
                        </p>
                    </div>
                    <PasswordField
                        id="confirm"
                ...
                {msgKey && (
                    <div className="rounded border border-green-200 bg-green-50 px-3 py-2 text-green-700 text-sm">
                        {t(msgKey)}
                    </div>
                )}
                {errKey && (
                ...
                <button
                    type="submit"
                    disabled={submitting}
                    className="rounded-lg px-4 py-2 disabled:opacity-60 transition hover:opacity-90"
                    style={{
                ...
                >
                    {submitting ? (
                        <LoadingDots label={t("admin.addMember.submitting")} />
                    ) : (
                        t("admin.addMember.submit")
                    )}
                </button>
            </form>
        </section>
