...
export default function DeleteMemberPage() {
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (Array.isArray(d))
                setErrKey("admin.deleteMember.errors.serverValidation");
            // membre non trouvé
            else if (ae?.status === 404)
                setErrKey("admin.deleteMember.errors.notFound");
            // action interdite
            else if (ae?.status === 403)
                setErrKey("admin.deleteMember.errors.forbidden");
            // erreur générique
    ...
    return (
        ...
        >
            ...
            <form className="space-y-4" onSubmit={onSubmit}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium mb-1">
                            {t("admin.deleteMember.firstNameLabel")}
                        </label>
                        <input
                            name="first_name"
                            ...
                            placeholder={t(
                                "admin.deleteMember.firstNamePlaceholder",
                            )}
                    ...
                    <div>
                        <label className="block text-sm font-medium mb-1">
                            {t("admin.deleteMember.lastNameLabel")}
                        </label>
                        <input
                            name="last_name"
                            ...
                            placeholder={t(
                                "admin.deleteMember.lastNamePlaceholder",
                            )}
                ...
                <div>
                    <label className="block text-sm font-medium mb-1">
                        {t("admin.deleteMember.emailLabel")}
                    </label>
                    <input
                        name="email"
                        type="email"
                ...
                <div>
                    <label className="block text-sm font-medium mb-1">
                        {t("admin.deleteMember.roleLabel")}
                    </label>
                    <select
                        name="role"
                        value={form.role}
                    ...
                    >
                        <option value="MEMBER">
                            {t("admin.deleteMember.roles.member")}
                        </option>
                        <option value="ADMIN">
                            {t("admin.deleteMember.roles.admin")}
                        </option>
                    </select>
                </div>
                {/* Messages */}
                ...
                >
                    {submitting ? (
                        <LoadingDots
                            label={t("admin.deleteMember.submitting")}
                        />
                    ) : (
                        t("admin.deleteMember.submit")
                    )}
                </button>
            </form>
        </section>
