...
export default function ConfigPlaceOfInterestPage() {
    ...
    const { items, loading, remove, reload } =
        useConfigResource<PlaceOfInterestDTO>(PlaceOfInterestAPI);
    const [editing, setEditing] = useState<PlaceOfInterestDTO | null>(null);
    const [creating, setCreating] = useState(false);

    const [placeOfInterestToDelete, setPlaceOfInterestToDelete] =
        useState<PlaceOfInterestDTO | null>(null);
    ...
    return (
        <div>
            <div className="mb-4 flex items-center justify-between">
                ...
                >
                    {t("admin.config.placeOfInterestPage.title")}
                </h2>
                <button
            ...
            {loading ? (
                <div className="text-sm" style={{ color: hoverText07 }}>
                    <LoadingDots
                        label={t("admin.config.placeOfInterestPage.loading")}
                    />
                </div>
            ) : (
                <div
                ...
                >
                    <table className="min-w-full text-sm">
                        ...
                        >
                            <tr>
                                <th className="px-3 py-2 text-left">
                                    {t(
                                        "admin.config.placeOfInterestPage.columns.name",
                                    )}
                                </th>
                                <th className="px-3 py-2 text-left">
                                    {t(
                                        "admin.config.placeOfInterestPage.columns.fr",
                                    )}
                                </th>
                                <th className="px-3 py-2 text-left">
                                    {t(
                                        "admin.config.placeOfInterestPage.columns.de",
                                    )}
                                </th>
                                <th className="px-3 py-2 text-left">
                                    {t(
                                        "admin.config.placeOfInterestPage.columns.it",
                                    )}
                                </th>
                                <th className="px-3 py-2 text-left">
                                    {t(
                                        "admin.config.placeOfInterestPage.columns.rm",
                                    )}
                                </th>
                                <th className="px-3 py-2 text-left">
                                    {t(
                                        "admin.config.placeOfInterestPage.columns.en",
                                    )}
                                </th>
                                <th className="px-3 py-2 text-left">
                                    {t(
                                        "admin.config.placeOfInterestPage.columns.position",
                                    )}
                                </th>
                      ...
                      `}
                                        ...
                                        >
                                            {t(
                                                "admin.config.placeOfInterestPage.edit",
                                            )}
                                        </button>
                                        <button
                                        ...
                                        >
                                            {t(
                                                "admin.config.placeOfInterestPage.delete",
                                            )}
                                        </button>
                                    </td>
                                </tr>
                            ...
                            {items.length === 0 && (
                                <tr>
                                    ...
                                    >
                                        {t(
                                            "admin.config.placeOfInterestPage.empty",
                                        )}
                                    </td>
                                </tr>
                            )}
            ...
            <ConfirmModal
                ...
                message={
                    placeOfInterestToDelete
                        ? t("admin.config.placeOfInterestPage.deleteConfirm", {
                              name: placeOfInterestToDelete.default_name,
                          })
                        : ""
                }
                confirmLabel={t(
                    "admin.config.placeOfInterestPage.confirmLabel",
                )}
