...
import ResetSwissControl, {
    SWISS_BOUNDS,
    SWISS_BOUNDS_PADDED,
} from "@/components/map/ResetSwissControl";
...
const ChoroplethDeckLayer = lazy(
    () => import("@/features/geo/3d/ChoroplethDeckLayer"),
);
...
export default function GeoJsonMap({
    ...
    const handleActivate3D = useCallback(() => {
        ...
        if (map) {
            ...
            initialDeckViewStateRef.current = {
                lng: c.lng,
                lat: c.lat,
                zoom: map.getZoom(),
            };
    ...
    const handleManualReturn2D = useCallback(() => {
        ...
        const map = (window as any).__leafletMap;
        if (map) {
            map.setView([vs.latitude, vs.longitude], vs.zoom, {
                animate: false,
            });
            requestAnimationFrame(() => map.invalidateSize(false));
        }
    ...
    function getMultiStripePattern(
        map: any,
        colors: string[],
        angle = 45,
        stripe = 6,
    ...
    useEffect(() => {
        ...
        async function loadBundle() {
            ...
            if (cached) {
                const isFresh =
                    Date.now() - cached.cachedAt < BASE_MAP_CACHE_TTL_MS;
    ...
    const activeTileConfig =
        basemap !== "none" ? BASEMAP_CONFIG[basemap] : null;
    ...
    return (
        ...
        >
            ...
            {choropleth && (
                <Map3DControl
                    ...
                    onToggle={
                        is3DMode ? handleManualReturn2D : handleActivate3D
                    }
                />
            )}
      ...
      `}</style>
                ...
                >
                    ...
                    <Pane name="pane-raster" style={{ zIndex: 100 }}>
                        {activeTileConfig && (
                            <TileLayer
                                ...
                                {...(activeTileConfig.subdomains != null
                                    ? {
                                          subdomains:
                                              activeTileConfig.subdomains,
                                      }
                                    : {})}
                    ...
                    {choropleth?.feature_collection && (
                        <>
                            ...
                            <GeoJSON
                                key={`choropleth-${choropleth.question_uid}-${
                                    choropleth.year_requested
                                }-${choropleth.granularity}-${
                                    basemap !== "none" ? "bm" : "no-bm"
                                }`}
                                ...
                                style={(feat: any) => {
                                    ...
                                    if (
                                        ...
                                        const angle =
                                            typeof pat.angle === "number"
                                                ? pat.angle
                                                : 45;
                                        const stripe =
                                            typeof pat.stripe === "number"
                                                ? pat.stripe
                                                : 6;

                                        const p = getMultiStripePattern(
                                            map,
                                            pat.colors,
                                            angle,
                                            stripe,
            ...
            {/* end Leaflet visibility wrapper */}
