...
export default function PageAll({
    ...
    const [editConfirmTarget, setEditConfirmTarget] =
        React.useState<AllItem | null>(null);
    ...
    React.useEffect(() => {
        const availableSortKeys = sortableColumns.map(
            (col) => col.sortKey ?? col.key,
        );
    ...
    const findPageForUid = React.useCallback(
        async (uid: number): Promise<number> => {
            try {
                const json = await apiFetch<FindPageResponse>(
                    "/pageAll/find_page",
                    {
                        method: "GET",
                        auth: true,
    ...
    const hasActions =
        !!actions && (actions.show || actions.edit || actions.delete);

    // Constante pour UI
    const controlCls =
        "h-9 rounded-lg border px-3 text-sm transition inline-flex items-center gap-1";
        ...
        >
            ...
            <div className="flex-1 flex flex-col">
                {loading && (
                    ...
                    >
                        <LoadingDots
                            label={t("dashboardSidebar.pageAll.loading")}
                        />
                    </div>
                )}
                {error && (
                            ...
                            <thead style={{ backgroundColor: hoverPrimary04 }}>
                                <tr>
                                    ...
                                    {hasActions && (
                                        ...
                                        >
                                            {t(
                                                "dashboardSidebar.pageAll.actions",
                                            )}
                                        </th>
                                    )}
                                </tr>
                            ...
                            <tbody>
                                {items.length === 0 && !loading ? (
                                    <tr>
                                        ...
                                        >
                                            {t(
                                                "dashboardSidebar.pageAll.noData",
                                            )}
                                        </td>
                                    </tr>
                                ) : (
                                    items.map((row, index) => (
                                        ...
                                        <tr
                                            ...
                                            className={[
                                                "transition-colors",
                                                selectedUid === row.uid
                                                    ? "bg-[var(--pageall-row-selected-bg)]"
                                                    : "",
                                                "hover:[background-color:var(--pageall-row-hover-bg)]",
                                            ].join(" ")}
                                            style={
                                                {
                                                    "--pageall-row-hover-bg":
                                                        hoverPrimary04,
                                                    "--pageall-row-selected-bg":
                                                        hoverPrimary15,
                                                } as React.CSSProperties
                                            }
                                        >
                                            ...
                                            {columns.map((col) => {
                                                ...
                                                const content = col.render
                                                    ? col.render(row)
                                                    : value ?? "\u2014"; // symbole unicode pour cela "—"
                                                const alignClass =
                                                    col.align === "center"
                                                        ? "text-center"
                                                        : col.align === "right"
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <PageAllEditableCell
                                                            ...
                                                            content={content}
                                                            isEditing={
                                                                inlineEdit.isEditingRow(
                                                                    row,
                                                                ) &&
                                                                inlineEdit.canEditColumn(
                                                                    col,
                                                                )
                                                            }
                                                            draftValue={
                                ...
                                `}
                                                                ...
                                                                onClick={() => {
                                                                    ...
                                                                    navigate(
                                                                        `/admin/places/show/${entity}/${row.uid}`,
                                                                    );
                                                                }}
                                                            >
                                                                {t(
                                                                    "dashboardSidebar.pageAll.show",
                                                                )}
                                                            </button>
                                                        )}
                                                        {actions?.edit && (
                                ...
                                `}
                                                            ...
                                                            >
                                                                {inlineEdit.isEditingRow(
                                                                    ...
                                                                    ? inlineEdit.loading
                                                                        ? t(
                                                                              "dashboardSidebar.pageAll.saving",
                                                                          )
                                                                        : t(
                                                                              "dashboardSidebar.pageAll.save",
                                                                          )
                                                                    : t(
                                                                          "dashboardSidebar.pageAll.edit",
                                                                      )}
                                                            </button>
                                                        )}
                                                        {actions?.delete && (
                                                            ...
                                                            >
                                                                {inlineEdit.isEditingRow(
                                                                    ...
                                                                    : t(
                                                                          "dashboardSidebar.pageAll.delete",
                                                                      )}
                                                            </button>
                                                        )}
                                                    </div>
                ...
                <ConfirmModal
                    ...
                    onConfirm={async () => {
                        ...
                        const ok =
                            await inlineEdit.confirmEditing(editConfirmTarget);
                        if (!ok) return;
