...
export default function MapLegendOverlay({ choropleth }: Props) {
    const { background, borderColor, textColor, hoverText07, hoverPrimary04 } =
        useTheme();
    ...
    const stats = useMemo(() => {
        ...
        if (
            ...
            !Array.isArray(choropleth?.feature_collection?.features)
        ) {
            return null;
        }
        ...
        const noData = features.filter(
            (f: any) => f?.properties?.value_kind === "no_data",
        ).length;
        const noResponse = features.filter(
            (f: any) => f?.properties?.value_kind === "no_response",
        ).length;
                                ...
                                <div className="flex items-center justify-between gap-4 mb-4">
                                    <div>
                                        <div className="text-sm font-semibold">
                                            {t("map.legend.distribution")}
                                        </div>
                                        <div
                                            className="text-xs"
                                            style={{ color: hoverText07 }}
                                        >
                                            {t("map.legend.unitsWithResponse", {
                                                count: stats.totalValid,
                                            })}
                                        </div>
                                    </div>
