...
const normalizeGeoCode = (value: unknown): string =>
    String(value ?? "")
        .trim()
        .toLowerCase();

const getLocalUid = (place: PlaceOfInterestMarker): number | null => {
    if (place.source !== "local") {
        return null;
    }

    const match = place.code.match(/^local-(commune|district|canton)-(\d+)$/);
...
const findGeoFeature = (
    ...
    if (place.geoCode) {
        ...
        const featureByCode = collection.features.find(
            (feature) =>
                normalizeGeoCode(feature.properties?.code) === normalizedCode,
        );
     ...
     * local-canton-8
     */
    const localUid = getLocalUid(place);
    ...
    return collection.features.find(
        (feature) => Number(feature.properties?.uid) === localUid,
    );
};

const createPositionMarkerIcon = (color: string): L.DivIcon =>
    L.divIcon({
...
export default function PlaceOfInterestMarkers({
    ...
    const communePlaces = placeOfInterest.filter(
        (place) => place.geoType === "commune",
    );

    const districtPlaces = placeOfInterest
        .filter((place) => place.geoType === "district")
        .map((place) => ({
            place,
            feature: findGeoFeature(districts, place),
        }))
        .filter(
            (item): item is AreaPlaceOfInterest => item.feature !== undefined,
        );

    const cantonPlaces = placeOfInterest
        .filter((place) => place.geoType === "canton")
        .map((place) => ({
            place,
            feature: findGeoFeature(cantons, place),
        }))
        .filter(
            (item): item is AreaPlaceOfInterest => item.feature !== undefined,
        );
    ...
    const resolvedAreaCodes = new Set([
        ...districtPlaces.map(({ place }) => place.code),
        ...cantonPlaces.map(({ place }) => place.code),
    ]);

    const pointPlaces = placeOfInterest.filter(
        (place) =>
            place.geoType === "commune" || !resolvedAreaCodes.has(place.code),
    );
    ...
    const bindAreaTooltip =
        (place: PlaceOfInterestMarker) =>
        (_feature: GeoFeature, layer: L.Layer) => {
            const tooltipLayer = layer as L.Path;
            tooltipLayer.bindTooltip(place.name, {
                sticky: true,
                direction: "right",
                offset: [8, 0],
                opacity: 1,
    ...
    return (
      ...
      `}</style>
            ...
            <Pane name="placeOfInterestDistricts" style={{ zIndex: 725 }}>
                {districtPlaces.map(({ place, feature }) => (
                    <GeoJSON
                        key={place.code}
                        data={feature}
                        pane="placeOfInterestDistricts"
                        style={() => districtStyle}
                        onEachFeature={bindAreaTooltip(place)}
                    />
                ))}
            </Pane>

            <Pane name="placeOfInterestCantons" style={{ zIndex: 726 }}>
                {cantonPlaces.map(({ place, feature }) => (
                    <GeoJSON
                        key={place.code}
                        data={feature}
            ...
            <Pane name="placeOfInterestPoints" style={{ zIndex: 725 }}>
                {pointPlaces.map((place) => (
                    <Marker
                        key={place.code}
                        position={place.pos as LatLngExpression}
              ...
              "
                ...
                "
                                style={{
                                    ...
                                    borderWidth: 1,
                                    borderStyle: "solid",
                                    boxShadow: `0 1px 2px ${hexToRgba(
                                        "#111827",
                                        0.1,
                                    )}`, // couleur en dure pour garder une coehérence visuelle
                                }}
                            >
                                {place.name}
                            </div>
