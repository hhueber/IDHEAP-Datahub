...
export default function Navbar() {
    ...
    useEffect(() => {
        const onClick = (e: MouseEvent) => {
            ...
            if (
                open &&
                panelRef.current &&
                !panelRef.current.contains(n) &&
                btnRef.current &&
                !btnRef.current.contains(n)
            )
                setOpen(false);
        };
    ...
    const current =
        langs.find((l) => curLang.startsWith(l.code) || curBase === l.code) ??
        langs[0];
    ...
    return (
        <>
            ...
            <button
                ...
                style={
                    { "--leaflet-top-offset": "96px" } as React.CSSProperties
                }
            >
                <img
                    src={logoUrl}
                ...
                {!open && (
                    <svg
                        viewBox="0 0 24 24"
                        className="absolute w-6 h-6 -right-2 -bottom-2"
                        aria-hidden="true"
                        style={{ color: primary }}
                    ></svg>
                )}
            ...
            {open && (
                <div className="fixed inset-0 z-[3450]">
                    <div
                        ...
                        style={{ backgroundColor: navbarOverlayBg }}
